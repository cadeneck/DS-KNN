...
class Leaf(Node):
    ...
    def predict(self, X: ArrayLike) -> np.ndarray:
        ...
        # >>> YOUR CODE HERE >>>
        #labels = np.array([self.class_labels[np.argmax(probabilities, axis=1)]])
...
class DecisionTree:
    ...
    def _build_tree(self, X: ArrayLike, y: ArrayLike, max_depth: int, exclude: set = set()) -> Node:
        ...
        # >>> YOUR CODE HERE >>>        
        feature_of_split, splits =  None, None

        for feature in range(X.shape[1]):
            unqiue_values = set(X[:, feature])

            if self.scorer.type == 'information':
                feature_gain = self.scorer.compute_information_gain(X, y, feature)
            else:
                feature_gain = self.scorer.compute_gini_gain(X, y, feature)
        
            feature_of_split = feature

            for value in unqiue_values:
                indices = (X[:, feature] == value).nonzero()[0]
        ...
        for feature_value, (X_split, y_split) in splits.items():
            ...
            # >>> YOUR CODE HERE >>>
            unqiue_values = np.unique(X_split[:, feature_of_split])

            if(len(unqiue_values) == 1):
                class_probabilities = self.scorer.compute_class_probabilities(y_split)
                node = Leaf(class_probabilities)
            else:
        ...
        if 'NA' not in children:
            # >>> YOUR CODE HERE >>>
            na_indices = (X[:, feature_of_split] == 'NA').nonzero()[0]
            class_probabilities = self.scorer.compute_class_probabilities(y[na_indices])
